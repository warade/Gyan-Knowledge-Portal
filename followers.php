...
<body>
	...
	<div id="userscontent" class="list-group">
	    ...
	    <tbody>
		<?php
		$followerids = get_follower_id($_SESSION['UserID']);
		foreach ($followerids as $key => $value) {
			echo "<tr>";
			$userid=$value['FollowerID'];
			$info = get_info($value['FollowerID']);
			foreach ($info as $key => $data) {
				echo "<td>".$data['Username']."</td>";
				echo "<td><a href='view_details.php?UserID=$userid'>view</a></td>";
